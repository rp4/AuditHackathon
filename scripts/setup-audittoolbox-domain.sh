...
echo "🔍 Checking if audittoolbox.com is verified..."
if gcloud domains list-user-verified --project=toolbox-478717 | grep -q "audittoolbox.com"; then
    echo "✅ Domain verified! Setting up domain mapping..."

    # Create domain mapping for root domain
    echo "Mapping audittoolbox.com to Cloud Run..."
    gcloud beta run domain-mappings create \
        --service=openauditswarms \
        --domain=audittoolbox.com \
        --region=us-central1

    # Create domain mapping for www
    echo "Mapping www.audittoolbox.com to Cloud Run..."
    gcloud beta run domain-mappings create \
        --service=openauditswarms \
        --domain=www.audittoolbox.com \
        --region=us-central1

    echo ""
    echo "📋 DNS Records to add to your provider:"
    echo "=================================="
    gcloud beta run domain-mappings describe --domain=audittoolbox.com --region=us-central1 2>/dev/null
    ...
    echo "   gcloud run services update openauditswarms --region=us-central1 --update-env-vars=NEXTAUTH_URL=https://audittoolbox.com"

else
    echo "❌ Domain not verified yet!"
    echo ""
    echo "Please verify audittoolbox.com:"
    echo "1. Go to: https://search.google.com/search-console"
    echo "2. Add property: audittoolbox.com"
